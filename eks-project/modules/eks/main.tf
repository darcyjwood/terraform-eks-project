...
resource "aws_eks_cluster" "eks-cluster-project" {
  name     = var.cluster_name
  role_arn = aws_iam_role.project.arn
  ...
  depends_on = [
    aws_iam_role_policy_attachment.project-AmazonEKSClusterPolicy,
    aws_iam_role_policy_attachment.project-AmazonEKSVPCResourceController,
  ]
}

resource "aws_eks_node_group" "project" {
  cluster_name    = aws_eks_cluster.eks-cluster-project.name
  node_group_name = var.node_group_name
  node_role_arn   = aws_iam_role.project2.arn
  subnet_ids      = var.aws_public_subnet
  ...
  depends_on = [
    aws_iam_role_policy_attachment.project-AmazonEKSWorkerNodePolicy,
    aws_iam_role_policy_attachment.project-AmazonEKS_CNI_Policy,
    aws_iam_role_policy_attachment.project-AmazonEC2ContainerRegistryReadOnly,
  ]
}
...
resource "aws_iam_role" "project2" {
  name = "eks-node-group-project2"
...
resource "aws_iam_role_policy_attachment" "project-AmazonEKSWorkerNodePolicy" {
  ...
  role       = aws_iam_role.project2.name
}
